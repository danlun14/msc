...
int basic_string_parser_first(char *str, int *i, unit_command *unit_commands, int *add_oper, char *name_var)
{
    ...
    for (int j = 0; isalpha(str[*i]); (*i)++, j++)
    {
        if (!isupper(str[*i]))
        {
            printf("Incorrect command. Command not must be in lowercase.");
            return 1;
        }
        command_[j] = str[*i];
    }
    ...
    {
        printf("Incorrect command. Command not found");
        return 1;
    }
...
int basic_translator(char *path_from, char *path_where, int *i_)
{
    ...
    *i_ = 0;
    ...
    while (getline(&buf, &len, in) != -1)
    {
        ...
        if (add_oper) // если есть сложная операция начиаем переодить ее в ассемблер
        {
            ...
            case GOTO:
                ...
                if (isCommandInPull(pull_commands, dig))
                {
                    ...
                    {
                        printf("Error GOTO. Can't get assembler line.\n");
                        return 1;
                    }
            ...
            case IF:
                ...
                if (isCommandInPull(pull_commands, num_cell_for_jump))
                {
                    ...
                    {
                        printf("Error IF. Can't get line assembler\n");
                        return 1;
                    }
                    if (add_var(oper_a, get_cellNumberForNewVariables()))
                    {
                        printf("Error IF. Can't add variable \n");
                        return 1;
                    }
                    var *vra = get_var(oper_a);
                    if (!vra)
                    {
                        printf("Error IF. Can't get variable\n");
                        return 1;
                    }
                    if (pull_commands[real_line].num_line < 10)
                    {
    ...
    for (int j = 0; j < real_line; j++)
    {
        ...
        if (j > 0 && pull_commands[j - 1].orig_num_line != (pull_commands[j].orig_num_line - 10) && pull_commands[j - 1].orig_num_line != (pull_commands[j].orig_num_line - 20))
        {
        ...
        else if (pull_commands[j].command == IF_B)
        {
            ...
            if ((num_line_to_ass = get_num_line_to_ass_from_pull(pull_commands, num_cell_for_jump)) == -1)
            {
                printf("Error IF_B. Can't get line to assemler\n");
                return 1;
            }
            if (add_var(oper_a, get_cellNumberForNewVariables()))
            {
                printf("Error IF_B. Can't create variable\n");
                return 1;
            }
            var *vra = get_var(oper_a);
            if (!vra)
            {
                printf("Error IF_B. Can't get variable\n");
                return 1;
            }
            if (pull_commands[j].num_line < 10)
            {
...
int basic_translator_goto(char *str, int *dig, int *i)
{
    ...
    for (j = 0; str[*i] != '\0' && str[*i] != ' ' && str[*i] != '\n'; (*i)++, j++)
    {
        ...
        else
        {
            ...
            {
                printf("Error parsing GOTO. Incorrect format number");
                return 1;
            }
        }
    }
...
int get_num_line_to_ass_from_pull(unit_command *pull_commands, int num)
{
    ...
    {
        //printf("%d %d\n", num, pull_commands[i].orig_num_line);
